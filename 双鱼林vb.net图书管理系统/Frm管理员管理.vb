...
Partial Public Class Frm管理员管理
    ...
    ''' </summary>
    Public Sub dataBind()
        Dim ds As DataSet = New BU_UserInfo().SelectAdmin("U_RoleType=2")
        '查询管理员
    ...
    Private Sub button1_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Else
            Dim ds As DataSet = New BU_UserInfo().SelectAdmin("U_RoleType=2 and U_Sex=0 and U_Name like '%" & userName & "%' ")
            dataGridView1.DataSource = ds.Tables(0).DefaultView
        End If
    End Sub

    Private Sub toolStripButton4_Click(ByVal sender As Object, ByVal e As EventArgs) Handles button1.Click
        dataBind()
    End Sub
    '删除
    Private Sub toolStripLabel删除_Click(ByVal sender As Object, ByVal e As EventArgs) Handles toolStripLabel删除.Click
        If DirectCast(dataGridView1.SelectedRows, System.Windows.Forms.BaseCollection).Count > 1 Then
            ...
            Return
        Else
            If LoginRoler.U_Name = "admin" Then
                MessageBox.Show("无法删除终极管理员", "提示", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
                Return
            Else
                Dim UserName As String = dataGridView1.CurrentRow.Cells(1).Value.ToString()
                If LoginRoler.U_Name = UserName Then
    ...
    Private Sub treeView1_NodeMouseClick_1(ByVal sender As Object, ByVal e As TreeNodeMouseClickEventArgs) Handles treeView1.NodeMouseClick
        ...
        If data.Length = 2 Then
            ...
            If data(1) = "男" Then
                '查询普通用户
                ds = New BU_UserInfo().SelectAdmin("U_RoleType=2 and U_Sex=0")
            Else
                '查询普通用户
                ds = New BU_UserInfo().SelectAdmin("U_RoleType=2 and U_Sex=1")
            End If
            dataGridView1.DataSource = ds.Tables(0).DefaultView
        End If
    End Sub
